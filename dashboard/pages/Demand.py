...
def load_data():
    ...
    path = os.path.join(base, '..', '..', 'data', 'features', 'demand_features.parquet')
    return pd.read_parquet(os.path.normpath(path))
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=df['date'], y=df['demand'], mode='lines', name='Actual'))
fig.add_trace(go.Scatter(x=future_dates, y=forecast,  mode='lines', name='Forecast'))

if show_intervals:
    fig.add_trace(go.Scatter(x=future_dates, y=upper, mode='lines', name='Upper'))
    fig.add_trace(go.Scatter(x=future_dates, y=lower, mode='lines', name='Lower'))
